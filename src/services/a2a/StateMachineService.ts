...
export class StateMachineService implements ILiquidLLMService {
    ...
    async sendMessage(prompt: string): Promise<string> {
        try {
            const response = await fetch(`${this.baseUrl}/agents/state-machine/a2a`, {
                method: 'POST',
                headers: {
            ...
            // Extract data update (side-channel for flow state updates)
            ...
            return textPart ? textPart.text : "Flow updated.";

        } catch (error) {
            console.error('State Machine Error:', error);
            return "I'm having trouble connecting to the state machine. Please try again.";
        }
    }
}
