...
export class QAAgentService implements ILiquidLLMService {
    ...
    async sendMessage(prompt: string): Promise<string> {
        try {
            const response = await fetch(`${this.baseUrl}/agents/qa/a2a`, {
                method: 'POST',
                headers: {
            ...
            return textPart ? textPart.text : "Answer received.";

        } catch (error) {
            console.error('QA Agent Error:', error);
            return "I'm having trouble connecting to the Q&A agent. Please try again.";
        }
    }
}
